...
系统 SHALL 在用户无参数运行 `tread` 时进入 dashboard TUI，作为继续阅读和打开新读物的入口。
...
Dashboard SHALL provide an `o` open prompt that accepts URLs and local paths without leaving the TUI.

#### Scenario: Open prompt appears
- **WHEN** the user presses `o` from dashboard list mode
- **THEN** dashboard SHALL show an input prompt for a URL or local path
