...
2. **🔍 Figma design check (MANDATORY — do NOT skip)**

   Before creating the change, perform two checks:

   a. Check `.claude/figma-links.json` for pending Figma links:
   ```bash
   cat .claude/figma-links.json 2>/dev/null || echo '{"pending":[]}'
   ```

   b. Regardless of file check result, **MUST ask the user** about Figma designs:
   ...
   - If figma-links.json has pending links and user provides no new link → use existing links
...
4. **Invoke /figma-spec to parse design (if Figma links exist)**
   ...
   - For each link, invoke `/figma-spec <link> --change <name>` via the Skill tool
